...
public class WorkerService(ApplicationDbContext context)
{
    ...
    public async Task<WorkerDto> CreateWorker(CreateWorkerDto worker)
    {
        ...
        if (edad < 18)
        {
            throw new InvalidOperationException("El trabajador debe ser mayor de 18 a�os");
        }
    ...
    public async Task<WorkerDto> UpdateWorker(UpdateWorkerDto worker)
    {
        var existingWorker = await _context.Trabajadores.FindAsync(worker.Id);

        if (existingWorker == null)
        {
            throw new InvalidOperationException($"No se encontró el trabajador con ID {worker.Id}");
        }

        var duplicateDoc = await _context.Trabajadores
            .FirstOrDefaultAsync(w => w.NumeroDocumento == worker.NumeroDocumento && w.Id != worker.Id);
    ...
    public async Task DeleteWorkerAsync(int id)
    {
        ...
        if (worker == null)
        {
            throw new InvalidOperationException($"No se encontr� el trabajador con ID {id}");
        }
